...
with col_table:
    ...
    display_cols = ["company_id", "company_name", "ROE", "ROCE", "Net Profit Margin", "D/E",
                     "FCF", "PAT CAGR 5yr", "Revenue CAGR 5yr", "composite_quality_score", "is_benchmark"]
    ...
    styled = table.drop(columns=["is_benchmark"], errors="ignore")
    if "is_benchmark" in table.columns:
        styled = table.style.apply(highlight_benchmark, axis=1).format(precision=2)
